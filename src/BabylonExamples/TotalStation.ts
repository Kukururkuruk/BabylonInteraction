...
setupZoomEffect(): void {
    ...
    const zoomedFov = defaultFov / 4; // Уменьшаем FOV для зума
    ...
    this.scene.onPointerObservable.add((pointerInfo) => {
        if (pointerInfo.type === PointerEventTypes.POINTERDOWN && pointerInfo.event.button === 2) {
            // Включаем зум
            if (!isZoomed) {
                this.camera.fov = zoomedFov; // Уменьшаем FOV
                isZoomed = true;
            }
        } else if (pointerInfo.type === PointerEventTypes.POINTERUP && pointerInfo.event.button === 2) {
            // Выключаем зум
            if (isZoomed) {
                this.camera.fov = defaultFov; // Возвращаем стандартное FOV
                isZoomed = false;
            }
        }
    });
